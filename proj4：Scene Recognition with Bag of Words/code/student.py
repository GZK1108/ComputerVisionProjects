...
def get_tiny_images(image_paths):
    ...
    for each in image_paths:
        ...
        image = image.flatten()  # 1xd = size * size
...
def build_vocabulary(image_paths, vocab_size):
    ...
    Outputs:
        ...
        a vocab_size x (z*z*9) (见下文)数组，包含K Means聚类产生的聚类中心。
...
def get_bags_of_words(image_paths):
    ...
    # Note that the blocks used need to be the same size as the build_vocabulary above,
    # i.e. the cels and pixels are set the same
