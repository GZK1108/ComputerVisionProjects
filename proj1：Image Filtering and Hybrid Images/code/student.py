...
def my_imfilter(image, filter):
    ...
    #                                            Your Code                                              #
    #####################################################################################################
    # print shape of filter and image
    print("image shape: ", image.shape)
    print("filter shape: ", filter.shape)
    # Create a matrix based on image size to store convolution results
    filtered_image = np.zeros(image.shape)
    # Convolutional operation, same filtering, ensuring consistent size
    for n in range(3):
        filtered_image[:, :, n] = correlate2d(image[:, :, n], filter, 'same')
    # print shape of filtered_image
    print("filtered_image shape: ", filtered_image.shape)
    assert filtered_image is not None  # if filtered_image is None, error. In this project, useless
    #####################################################################################################
    #                                               End                                                 #
    #####################################################################################################
...
def gen_hybrid_image(image1, image2, cutoff_frequency):
  ...
  """
    ...
    kernel = np.outer(probs, probs)  # gaussian kernel
    ...
    # Your code here #
    low_frq_image2 = my_imfilter(image2, kernel)

    high_frequencies = image2 - low_frq_image2  # Replace with your implementation
